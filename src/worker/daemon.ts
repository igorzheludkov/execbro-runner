...
import { PATHS, slotLockPath } from "../config/paths.js";
...
async function tryRunNext(config: Config): Promise<void> {
    ...
    const release = await tryClaimSlot(slotLockPath(iosSlot.id));
    if (!release) return;
    ...
    let outcome;
    try {
        outcome = await runTask(descriptor, iosSlot, config);
    } catch (e) {
        outcome = { status: "failed" as const, reason: (e as Error).message };
    }
...
async function main(): Promise<void> {
    ...
    console.log(`[worker] started, ${config.slots.length} slot(s) configured`);
